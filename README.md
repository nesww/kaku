...
- [vesa.h](kernel/hw/vesa/vesa.h): VESA display with 8 bits colors
- [tty.h](kernel/tty/tty.h): basic TTY, only outputting, waiting for shell implementation 

Coming:
- Kernel internal logging system for infos & debugging, to be extracted from the VM (serial output for now)
- Repair IOs (VGA & Serial) for weird race condition from processes (VGA is deprecated, to check)
