...
void tty_flush_current_line(void) {
    SERIAL_INFO("will flush current line\n");
    __tty_flush_line(tty_cursor_y);
}

uint32_t tty_get_cursor_y(void) {
    return tty_cursor_y;
}

void tty_flush_from(uint32_t from_line) {
    for (uint32_t line = from_line; line <= tty_cursor_y; line++)
        __tty_flush_line(line);
}
