...
void fs_ext2_read_superblock(void) {
    ...
    if (!ata_read(FS_MBR_LBA, 1, buf_mbr)) {
        panic("ext2_read_superblock: failed to read MBR");
    }
    ...
    if (!ata_read(lba_ext2_sb, 2, buf_lba_ext2)) {
        panic("ext2_read_superblock: failed to read superblock from disk");
    }
...
void fs_ext2_read_bgdt(void) {
    ...
    if (!ata_read(__fs_ext2_lba_root + 4, CEIL_DIV(bgdt_size, ATA_SECTOR_SIZE), buf_bgdt)) {
        panic("ext2_read_bgdt: could not read bgdt from disk");
    }
...
static void __fs_ext2_allread(const char *func_name) {
    if (!__fs_ext2_sb_read)
        panicf("%s: could not do operation, superblock was not read\n", func_name);
    if (!__fs_ext2_bgdt_read)
        panicf("%s: could not do operation, BGDT was not read\n", func_name);
}
...
fs_ext2_inode fs_ext2_read_inode(uint32_t inode_id) {
    ...
    if (!ata_read(inode_lba, __fs_ext2_block_size/ATA_SECTOR_SIZE, buf_inode)) {
        SERIAL_WARN("driver could not read inode %d from disk\n", inode_id);
...
uint8_t fs_ext2_write_inode(fs_ext2_inode *inode) {
    __fs_ext2_allread(__func__);
    ...
    if (!ata_write(inode_lba, __fs_ext2_block_size/ATA_SECTOR_SIZE, buf_inode)) {
        SERIAL_WARN("driver could not write inode %d onto disk\n", inode->inode_id);
        return FALSE;
    }
...
static uint32_t __fs_ext2_get_block_ptr(fs_ext2_inode *inode, uint32_t i) {
    if (i < 12) {
        return *(&inode->inode_direct_blk_ptr0 + i);
    }
    i -= 12;
    if (i >= __fs_ext2_block_size / 4) return 0;
    if (inode->inode_singly_indirect_blk_ptr == 0) return 0;

    uint32_t *indirect = kmalloc(__fs_ext2_block_size);
    fs_ext2_lba lba = FS_EXT2_BLK_TO_LBA(inode->inode_singly_indirect_blk_ptr);
    ata_read(lba, __fs_ext2_block_size / ATA_SECTOR_SIZE, (uint16_t*)indirect);
    uint32_t blk = indirect[i];
    kfree(indirect);
    return blk;
}

static uint32_t __fs_ext2_get_or_alloc_block_ptr(fs_ext2_inode *inode, uint32_t i) {
    if (i < 12) {
        uint32_t *direct = &inode->inode_direct_blk_ptr0;
        if (direct[i] == 0) {
            direct[i] = fs_ext2_alloc_block(0);
        }
        return direct[i];
    }
    i -= 12;
    ...
    if (inode->inode_singly_indirect_blk_ptr == 0) {
        inode->inode_singly_indirect_blk_ptr = fs_ext2_alloc_block(0);
    }
    ...
    if (indirect[i] == 0) {
        indirect[i] = fs_ext2_alloc_block(0);
        ata_write(lba, __fs_ext2_block_size / ATA_SECTOR_SIZE, (uint16_t*)indirect);
    }
...
dynarray *fs_ext2_list_dir(fs_ext2_inode *inode) {
    ...
    for (uint32_t i = 0; i < nb_blocks; ++i) {
        uint32_t blk = __fs_ext2_get_block_ptr(inode, i);
        fs_ext2_lba lba     = FS_EXT2_BLK_TO_LBA(blk);
        fs_ext2_ata_buf buf = kmalloc(__fs_ext2_block_size);
        if(!ata_read(lba, __fs_ext2_block_size / ATA_SECTOR_SIZE, buf)) {
            SERIAL_WARN("driver could not read from disk\n");
            return 0;
        }
...
int32_t __fs_ext2_find_in_dir(fs_ext2_inode *dir, const char *name) {
    uint32_t inode_id = 0;
...
fs_ext2_inode fs_ext2_resolve_path(const char* path) {
    ...
    //todo: check fs_root existence here
...
int fs_ext2_read_file_contents(fs_ext2_inode *inode, uint8_t *buf, uint32_t size) {
    ...
    while (i < nb_blocks && written_bytes < size) {
        ...
        if (!ata_read(lba, __fs_ext2_block_size / ATA_SECTOR_SIZE, buf_blk)) {
            SERIAL_WARN("could not read block %d when reading file\n", i);
            kfree(buf_blk);
            break;
        }
...
void __fs_ext2_write_bgdt_to_disk(void) {
    if (!ata_write(__fs_ext2_lba_root + 4, CEIL_DIV(__fs_ext2_nb_block_groups * sizeof(fs_ext2_descriptor), ATA_SECTOR_SIZE), (uint16_t*)fs_ext2_bgdt)) {
        SERIAL_WARN("could not write BGDT to disk\n");
    }
}

void __fs_ext2_write_sb_base_to_disk(void) {
    uint16_t *buf = kmalloc(__fs_ext2_block_size);
    if (!ata_read(__fs_ext2_lba_root + 2, __fs_ext2_block_size / ATA_SECTOR_SIZE, buf)) {
        SERIAL_WARN("could not read superblock for update\n");
        kfree(buf);
        return;
    }
    kmemcpy(buf, &fs_ext2_sb.base, sizeof(fs_ext2_superblock_base));
    if (!ata_write(__fs_ext2_lba_root + 2, __fs_ext2_block_size / ATA_SECTOR_SIZE, buf)) {
        SERIAL_WARN("could not write superblock to disk\n");
    }
    kfree(buf);
}
...
uint32_t fs_ext2_alloc_block(uint32_t preferred_group) {
    ...
    for (uint32_t blk = 0; blk < __fs_ext2_nb_block_groups; ++blk) {
        ...
        if (desc.dsc_unallocated_blocks_count_in_group > 0) {
            ...
            if (!ata_read(lba, __fs_ext2_block_size / ATA_SECTOR_SIZE, buf_bitmap)) {
                SERIAL_WARN("could not read block bitmap in alloc_block\n");
                kfree(buf_bitmap);
                continue;
            }
            ...
            for (uint32_t byte_index = 0; byte_index < __fs_ext2_block_size; ++byte_index) {
                for (uint8_t bit_index = 0; bit_index < 8; ++bit_index) {
                    if (((block_bitmap[byte_index] >> bit_index) & 0b1) == 0) {
                        ...
                        if (!ata_write(lba, __fs_ext2_block_size / ATA_SECTOR_SIZE, buf_bitmap)) {
                            SERIAL_WARN("could not write block bitmap in alloc_block\n");
                            kfree(buf_bitmap);
                            continue;
                        }
...
uint32_t fs_ext2_alloc_inode(uint32_t preferred_group) {
    ...
    for (uint32_t blk = 0; blk < __fs_ext2_nb_block_groups; ++blk) {
        ...
        if (desc.dsc_unallocated_inodes_count_in_group > 0) {
            ...
            if (!ata_read(lba, __fs_ext2_block_size / ATA_SECTOR_SIZE, buf_bitmap)) {
                SERIAL_WARN("could not read inode bitmap in alloc_inode\n");
                kfree(buf_bitmap);
                continue;
            }
            ...
            for (uint32_t byte_index = 0; byte_index < fs_ext2_sb.base.sb_nb_inodes_in_group / 8; ++byte_index) {
                for (uint8_t bit_index = 0; bit_index < 8; ++bit_index) {
                    if (((inode_bitmap[byte_index] >> bit_index) & 0b1) == 0) {
                        ...
                        if (!ata_write(lba, __fs_ext2_block_size / ATA_SECTOR_SIZE, buf_bitmap)) {
                            SERIAL_WARN("could not write inode bitmap in alloc_inode\n");
                            kfree(buf_bitmap);
                            continue;
                        }
...
int fs_ext2_write_file(fs_ext2_inode *inode, uint8_t *buf, uint32_t size) {
    ...
    while (written_bytes < size) {
        ...
        uint32_t blk = __fs_ext2_get_or_alloc_block_ptr(inode, i);
        if (blk == 0) break;
        fs_ext2_lba lba = FS_EXT2_BLK_TO_LBA(blk);

        fs_ext2_ata_buf buf_blk = kmalloc(__fs_ext2_block_size);
        if (!ata_read(lba, __fs_ext2_block_size / ATA_SECTOR_SIZE, buf_blk)) {
            SERIAL_WARN("could not read block %d when writing file\n", i);
            kfree(buf_blk);
            break;
        }

        kmemcpy(buf_blk, buf+written_bytes, nb_of_bytes_to_copy);
        if (!ata_write(lba, __fs_ext2_block_size / ATA_SECTOR_SIZE, buf_blk)) {
            SERIAL_WARN("could not write block %d when writing file\n", i);
            kfree(buf_blk);
            break;
        }
...
void fs_ext2_create_file(const char *path, uint8_t *buf, uint32_t size) {
    ...
    uint32_t last_blk               = __fs_ext2_get_block_ptr(&parent_inode, parent_nb_blocks - 1);
    if (last_blk == 0) {
        SERIAL_WARN("could not get parent dir last block when creating file\n");
        return;
    }
    fs_ext2_lba lba_last_blk        = FS_EXT2_BLK_TO_LBA(last_blk);
    fs_ext2_ata_buf buf_last_blk    = kmalloc(__fs_ext2_block_size);
    if (!ata_read(lba_last_blk, __fs_ext2_block_size / ATA_SECTOR_SIZE, buf_last_blk)) {
        SERIAL_WARN("could not read parent dir last block when creating file\n");
        kfree(buf_last_blk);
        return;
    }
    ...
    if (!ata_write(lba_last_blk, __fs_ext2_block_size / ATA_SECTOR_SIZE, buf_last_blk)) {
        SERIAL_WARN("could not write parent dir last block when creating file\n");
    }
    kfree(buf_last_blk);
}
